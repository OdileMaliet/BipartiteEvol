...
sim.BipartiteEvoll=function(nx,ny=nx,NG,dSpace=1,D=1,muP,muH,alphaP=0,alphaH=0,iniP=0,iniH=0,nP=1,nH=1,rP=1,rH=1,f=3,effect=1,
                       verbose=100,timeStep=floor(sqrt(NG/(nH+nP))),R=1,oneByOne=F,thin=1,P=NULL,H=NULL){
  ...
  f=function(X,x,r,alpha,pos){
    .Call("fitnessFunction", X=as.numeric(t(X)), x=as.numeric(x),r=as.numeric(r),alpha=as.numeric(alpha), Ncol=as.integer(N), D=as.integer(D),
          dSpace=as.numeric(dSpace), Xs=as.numeric(Xs),Ys=as.numeric(Ys),I=as.integer(pos))
  }
...
define.species=function(genealogy,threshold=1,distanceH=NULL,distanceP=NULL, verbose=T){
